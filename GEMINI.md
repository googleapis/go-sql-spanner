...
- **State Tracking & Failed Transaction Semantics**: The driver tracks the active transaction using `delegatingTransaction`.
  - **Defensive Nil-Pointer Safety**: Whenever `rollbackBackendTransaction()` cleans up an aborted transaction and nils out backend transaction fields (`rwTx`, `roTx`, `boTx`), all methods on `readWriteTransaction` and `readOnlyTransaction` MUST perform defensive nil checks returning `codes.FailedPrecondition` to prevent nil-pointer panics across all driver usages.
  - **PostgreSQL Error State Enforcement (`InErrorTxBehaviorEnforceInErrorState`)**: By default, Go's `database/sql` driver initializes connections with `InErrorTxBehaviorAllowCommands` to avoid breaking `database/sql` or `go-gorm-spanner` users. Connections created via `spannerlib` (for Node.js, Python, Java, Ruby, .NET wrappers) automatically enable `InErrorTxBehaviorEnforceInErrorState` for PostgreSQL databases (or when explicitly set via `SetInErrorTxBehavior`). When `Commit()` is called on a transaction in state `'E'` under `EnforceInErrorState`, the driver MUST invoke `c.state.Rollback()` and `txResultRollback` to discard any buffered session variable changes (`SET ...`), matching PostgreSQL semantics.
