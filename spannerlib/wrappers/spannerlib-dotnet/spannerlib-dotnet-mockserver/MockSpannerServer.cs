...
public class ExecutionTime
{
    ...
    internal void SimulateExecutionTime()
    {
        if (_minimumExecutionTime > 0 || _randomExecutionTime > 0)
        {
            ...
            {
                totalWaitTime += _random?.Next(_randomExecutionTime) ?? _randomExecutionTime;
            }
            Thread.Sleep(totalWaitTime);
        }
        if (_exception != null && _exceptionStreamIndex < 0)
        {
            throw _exception;
        }
    }
    ...
    public static ExecutionTime FromException(Exception exception)
    {
        return new ExecutionTime(0, 0, exception, -1, null);
    }

    public static ExecutionTime CreateException(StatusCode code, string message)
    {
        return FromException(new RpcException(new GrpcCore.Status(code, message)));
    }

    public static ExecutionTime StreamException(Exception exception, int streamIndex, BlockingCollection<int> streamWritePermissions)
    {
...
public class MockSpannerService : Spanner.V1.Spanner.SpannerBase
{
    ...
    {
        private static readonly int SMaxRowsInChunk = 1;
        ...
        PartialResultSet IEnumerator<PartialResultSet>.Current => _current ?? throw new InvalidOperationException();
        ...
        public bool MoveNext()
        {
            ...
            int recordCount = 0;
            while (recordCount < SMaxRowsInChunk && _currentRow < _resultSet.Rows.Count)
            {
    ...
    private static readonly string SDialectQuery =
        "select option_value from information_schema.database_options where option_name='database_dialect'";
    private static readonly Empty SEmpty = new ();
    private static readonly TransactionOptions STransactionSingleUse = new() { ReadOnly = new TransactionOptions.Types.ReadOnly { Strong = true, ReturnReadTimestamp = false } };
    ...
    private Transaction? FindOrBeginTransaction(SessionName session, TransactionSelector? selector)
    {
        if (selector == null)
        {
            return BeginTransaction(session, STransactionSingleUse, true);
        }
    ...
    public override Task<BatchCreateSessionsResponse> BatchCreateSessions(BatchCreateSessionsRequest request, ServerCallContext context)
    {
        ...
        _contexts.Enqueue(context);
        _headers.Enqueue(context.RequestHeaders);
        _executionTimes.TryGetValue(nameof(BatchCreateSessions), out ExecutionTime? executionTime);
        executionTime?.SimulateExecutionTime();
    ...
    public override Task<Session> CreateSession(CreateSessionRequest request, ServerCallContext context)
    {
        ...
        _contexts.Enqueue(context);
        _headers.Enqueue(context.RequestHeaders);
        _executionTimes.TryGetValue(nameof(CreateSession), out ExecutionTime? executionTime);
        executionTime?.SimulateExecutionTime();
        var database = request.DatabaseAsDatabaseName;
            ...
            if (_results.TryGetValue(statement.Sql.Trim(), out StatementResult? result))
            {
                switch (result.Type)
                {
                    ...
                    case StatementResult.StatementResultType.Exception:
                        ...
                        response.Status = StatusFromException(result.Exception ?? new InvalidOperationException());
                        break;
                    default:
