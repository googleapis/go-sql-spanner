...
public class StreamingRows : Rows
{
    ...
    public static async Task<StreamingRows> CreateAsync(Connection connection, AsyncServerStreamingCall<RowData> stream)
    {
        var rows = new StreamingRows(connection, stream);
        rows._pendingRow = await rows.NextAsync();
        return rows;
    }
    ...
    public override async Task<ListValue?> NextAsync()
    {
        if (_pendingRow != null) {
            var row = _pendingRow;
            _pendingRow = null;
        ...
        try
        {
            var hasNext = await _stream.ResponseStream.MoveNext();
            if (!hasNext)
            {
                MarkDone();
                return null;
