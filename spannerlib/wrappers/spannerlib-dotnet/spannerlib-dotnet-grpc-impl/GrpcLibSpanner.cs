...
public class GrpcLibSpanner : ISpannerLib
{
    ...
    async Task<T> TranslateExceptionAsync<T>(Func<Task<T>> f)
    {
        try
        {
        ...
        {
            throw new SpannerException(new Status { Code = (int) exception.Status.StatusCode, Message = exception.Message });
        }
    }
    ...
    {
        await TranslateExceptionAsync(async () => await _client.CloseConnectionAsync(ToProto(connection)));
    }
    ...
    public async Task<Rows> ExecuteAsync(Connection connection, ExecuteSqlRequest statement)
    {
        if (_useStreamingRows) 
        {
            return await ExecuteStreamingAsync(connection, statement);
        }
        var rows = await TranslateExceptionAsync(async () => await _client.ExecuteAsync(new ExecuteRequest
        {
            Connection = ToProto(connection),
            ExecuteSqlRequest = statement,
        }));
    ...
    private async Task<StreamingRows> ExecuteStreamingAsync(Connection connection, ExecuteSqlRequest statement)
    {
        var client = _clients[Random.Shared.Next(_clients.Length)];
        var stream = TranslateException(() => client.ExecuteStreaming(new ExecuteRequest
        {
        ...
        return await StreamingRows.CreateAsync(connection, stream);
    }
    ...
    public async Task<long[]> ExecuteBatchAsync(Connection connection, ExecuteBatchDmlRequest statements)
    {
        var stats = await TranslateExceptionAsync(async () => await _client.ExecuteBatchAsync(new ExecuteBatchRequest
        {
            Connection = ToProto(connection),
            ExecuteBatchDmlRequest = statements,
        }));
    ...
    {
        return await TranslateExceptionAsync(async () => await _client.MetadataAsync(ToProto(rows)));
    }
    ...
    {
        return await TranslateExceptionAsync(async () => await _client.NextAsync(new NextRequest
        {
            Rows = ToProto(rows),
            NumRows = numRows,
            Encoding = (long) encoding,
    ...
    public async Task CloseRowsAsync(Rows rows)
    {
        await TranslateExceptionAsync(async () => await _client.CloseRowsAsync(ToProto(rows)));
    }
