...
public class Server : IDisposable
{
    ...
    public string Start(AddressType addressType = AddressType.UnixDomainSocket)
    {
        ...
        (_host, _process) = StartServer(addressType, TimeSpan.FromSeconds(5));
        return _host;
    }
    
    private static Tuple<string, Process> StartServer(AddressType addressType, TimeSpan timeout)
    {
        string arguments;
        ...
        if (addressType == AddressType.UnixDomainSocket)
        {
            ...
            while (!File.Exists(arguments))
            {
                ...
                {
                    throw new TimeoutException($"Attempt to start server timed out after {timeout}");
                }
                Thread.Sleep(1);
            }
        }
    ...
    private static string GetBinaryFileName()
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            switch (RuntimeInformation.OSArchitecture)
            {
                case Architecture.X64:
                    fileName = "runtimes/win-x64/native/spannerlib_socket_server.exe";
                    break;
            }
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            {
                case Architecture.X64:
                    fileName = "runtimes/linux-x64/native/spannerlib_socket_server";
                    break;
            }
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            switch (RuntimeInformation.ProcessArchitecture)
            {
                case Architecture.Arm64:
                    fileName = "runtimes/osx-arm64/native/spannerlib_socket_server";
                    break;
            }
        }
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            if (File.Exists("runtimes/any/native/spannerlib_socket_server.exe"))
            {
                return "runtimes/any/native/spannerlib_socket_server.exe";
            }
        }
        if (File.Exists("runtimes/any/native/spannerlib_socket_server"))
        {
