...
export class Rows {
  ...
  constructor(connection: Connection, oid: number) {
    if (!connection.pool || connection.pool.oid === null) {
      throw new Error(
        'Cannot create Rows: Connection is not bound to an active Pool'
      );
    }
    if (connection.oid === null) {
      throw new Error('Cannot create Rows: Connection is invalid');
    }
  ...
  async next(): Promise<unknown> {
    ...
    if (!this.connection.pool || this.connection.pool.oid === null) {
      throw new Error(
        'Connection must be bound to an active Pool to fetch rows'
      );
    }
