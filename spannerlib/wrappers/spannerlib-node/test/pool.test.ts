...
describe('Pool', () => {
    ...
    it('should create a Pool successfully', async () => {
        ...
        stub.onFirstCall().resolves({ objectId: 1, pinnerId: 2, protobufBytes: null });
        ...
        assert.strictEqual(stub.firstCall.args[0], 'CreatePool', 'First call should be CreatePool');
        
        // Verify user agent suffix detection
        const isESM = typeof require === 'undefined';
        const expectedUA = isESM ? 'node-esm' : 'node-cjs';
        assert.strictEqual(stub.firstCall.args[3], expectedUA, `User agent should be ${expectedUA}`);
    });
