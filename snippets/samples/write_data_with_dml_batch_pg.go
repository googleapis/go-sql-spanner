...
func WriteDataWithDmlBatchPostgreSQL(ctx context.Context, w io.Writer, databaseName string) error {
	...
	insert := "insert into singers (singer_id, first_name, last_name) " +
		"values ($1, $2, $3)"
	...
	if _, err := conn.ExecContext(ctx, insert, 16, "Sarah", "Wilson"); err != nil {
		return err
	}
	if _, err := conn.ExecContext(ctx, insert, 17, "Ethan", "Miller"); err != nil {
		return err
	}
	if _, err := conn.ExecContext(ctx, insert, 18, "Maya", "Patel"); err != nil {
		return err
	}
