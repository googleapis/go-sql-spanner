...
{
    internal static string GetLabel(this Program.BenchmarkType type)
    {
...
internal class Stats
{
    ...
    private readonly Dictionary<Program.BenchmarkType, Dictionary<string, string>> _operationLabels = new();
    ...
    private readonly Dictionary<Program.BenchmarkType, LinkedList<double>> _operationLatencies = new();
        ...
        bool directPath)
    {
        ...
        foreach (var type in Enum.GetValues<Program.BenchmarkType>())
        {
    ...
    internal void RegisterOperationLatency(Program.BenchmarkType benchmarkType, double millis)
    {
    ...
    {
        Dictionary<Program.BenchmarkType, List<double>> latencies = new Dictionary<Program.BenchmarkType, List<double>>();
        lock (_lock)
        {
    ...
    private Distribution CreateDistribution(Program.BenchmarkType benchmarkType, List<double> points)
    {
        if (points.Count == 0)
        {
        ...
        if (benchmarkType == Program.BenchmarkType.LargeQuery)
        {
            bounds =
            [
